...
export const ProductCard = memo(function ProductCard({
  ...
  const { accent, ready: accentReady } = useImageAccentColor(imageUrl, { enabled: hovered });
  const displayPrice = effectiveProductPrice(product);
  const showLaser = hovered && accentReady;
  ...
  return (
    ...
    >
      <div ref={cardRef} className="relative w-full">
        <CardLaserFrame accent={accent} hovered={showLaser}>
          <div
            className={cn(
            ...
            <div className="absolute inset-x-0 bottom-3 z-10 flex justify-center px-3">
              <button
                ...
                className={cn(
                  ...
                  "hover:scale-[1.04] hover:-translate-y-px active:scale-[0.96]",
